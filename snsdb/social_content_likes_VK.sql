...
FROM 
	...
	INNER JOIN paydb.sns_profiles on paydb.sns_profiles.sns_id = content.sns_id AND paydb.sns_profiles.sns_profile_id = content.sns_item_id_0
 	LEFT OUTER JOIN likes on content.sns_item_id_1 = likes.sns_object_id_1 AND content.sns_item_id_0 = likes.sns_object_id_0  -- GENERAL CASE
...
-- ****** LIKES ONLY FROM FRIENDS  ******
	AND 	likes.sns_object_id_0 IN -- NOT IN Очень долго обрабатывается
												(SELECT
...
WHERE 
	...
	AND content.sns_id = @sns_id
  AND (content.date <= FROM_UNIXTIME(paydb.sns_profiles.date_create) OR content.date IS NULL)
  AND likes.sns_object_id_0 = likes.sns_profile_id -- ****** AUTOLIKES ******

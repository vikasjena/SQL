...
DROP FUNCTION IF EXISTS SessResultCountUnionCurrent;
CREATE FUNCTION SessResultCountUnionCurrent (ID BIGINT, purchase_date BIGINT, result_text CHAR(30)) 
...
WHERE Vw_Sessions_Data.date_start_u <= purchase_date AND Vw_Sessions_Data.union_id_current = ID AND Vw_Sessions_Data.result = result_text;
RETURN _COUNT;
END;

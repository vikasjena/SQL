SELECT
	...
	DAY(FROM_UNIXTIME(max(credits.date_create))) as 'Day',
...
-- 	YEAR(FROM_UNIXTIME(max(credits.date_create))) AS Year_,
	...
	count(credits.id) AS All_Cred_Count,
	sum(credits.is_first_credit) AS New_Cred_Count,
...
format((((SELECT 
				...
				WHERE _YEAR = 	YEAR(FROM_UNIXTIME(max(credits.date_create))) AND _DAY = day(FROM_UNIXTIME(max(credits.date_create))) 
...
format((((SELECT 
				...
				(SELECT 
					...
					month(Vw_Sessions_Unions_By_Days.date_start) AS _MONTH, 					day(Vw_Sessions_Unions_By_Days.date_start) AS _DAY, 
				...
				GROUP BY
					...
					day(Vw_Sessions_Unions_By_Days.date_start)
...
format((((SELECT 
				...
				GROUP BY
					Vw_Sessions_Unions_By_Days.shop_id,
					year(Vw_Sessions_Unions_By_Days.date_start),
					month(Vw_Sessions_Unions_By_Days.date_start), 					day(Vw_Sessions_Unions_By_Days.date_start)
										) AS temp_table_1
				WHERE _YEAR = 	YEAR(FROM_UNIXTIME(max(credits.date_create))) AND _DAY = day(FROM_UNIXTIME(max(credits.date_create))) 
...
format((((SELECT 
				...
				(SELECT 
					...
					day(Vw_Sessions_Unions_By_Days.date_start) AS _DAY, 
					year(Vw_Sessions_Unions_By_Days.date_start) AS _YEAR
				FROM Vw_Sessions_Unions_By_Days
...
CONCAT(format((((SELECT 
				...
				WHERE _YEAR = 	YEAR(FROM_UNIXTIME(max(credits.date_create))) AND _DAY = day(FROM_UNIXTIME(max(credits.date_create))) AND  _ID = credits.shop_id  AND _MONTH = MONTH (FROM_UNIXTIME(max(credits.date_create)))) )
					/  
			((SELECT 
				_COUNT
				FROM 
...
-- AND FROM_UNIXTIME(credits.date_create) BETWEEN '2014-01-23 23:59:59' AND '2014-01-25 00:00:00'
	AND Vw_Loans_Issued_two.Date_Create > '2015-01-01'
GROUP BY
